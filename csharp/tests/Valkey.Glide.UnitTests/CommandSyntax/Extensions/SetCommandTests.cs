...
public class SetCommandTests
{
    ...
    {
        // Arrange
        var nativeClient = Substitute.For<INativeClient>();
        var glideSerializerCollection = new GlideSerializerCollection();
        ...
        nativeClient.SendCommandAsync(Arg.Any<ERequestType>(), Arg.Any<IRoutingInfo>(), Arg.Any<IParameter[]>())
        ...
        await Helpers.IgnoreExceptionAsync(
            async () =>
            {
                // ToDo: Integrate generic "T" into all methods
                args = args.Select(
                        e => e switch
                        {
                ...
                var methodInfo = typeof(SetCommands)
                    .GetMethods()
                    .Where(methodInfo => methodInfo.Name == method)
                    .Single(methodInfo => methodInfo.GetParameters().Skip(3)
                        .Select(parameterInfo => parameterInfo.ParameterType).SequenceEqual(types));
                Assert.NotNull(methodInfo);
                switch (methodInfo.MakeGenericMethod(typeof(string)).Invoke(null, [glideClient, ..args]))
                {
                    case Task<string> t:
                        await t;
                        break;
        ...
        IEnumerable<IParameter> parameters = (IEnumerable<IParameter>)call.GetArguments()[2]!;
